...
RouteBoxer.prototype.getGridIntersects_ = function (start, end, startXY, endXY) {
  ...
  var brng = this.rhumbBearingTo(start, end);         // Step 1.
...
*/ 
RouteBoxer.prototype.rhumbDestinationPoint = function (point, brng, dist) {
...
RouteBoxer.prototype.rhumbBearingTo = function (point, dest) {
  var dLon = (dest.lng() - point.lng()).toRad();
  var dPhi = Math.log(Math.tan(dest.lat().toRad() / 2 + Math.PI / 4) / Math.tan(point.lat().toRad() / 2 + Math.PI / 4));
